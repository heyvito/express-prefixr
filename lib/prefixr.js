      ...
      vendoredProperties.forEach(function(prop) {
        ...
        return css = css.replace(regex, function(match, spaces, prop, value) {
          ...
          if (value.indexOf('linear-gradient' > -1)) {
            value = value.replace(/(\s|:|,)(repeating-)?linear-gradient\(\s*(-?\d*\.?\d*)deg/ig, function($0, delim, repeating, deg) {
              repeating = repeating || '';
              return "" + delim + repeating + "linear-gradient(" + (90 - deg) + "deg)";
            });
          }
          vendors.forEach(function(vendor) {
            return result += "" + spaces + vendor + prop + ": " + value + ";\n";
          });
          return result;
        });
      });
